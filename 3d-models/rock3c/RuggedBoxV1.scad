...
include <camera-case-cubiea7s.scad>
...
internalBoxWidthXMm = 130; // .1
// The length(Y) of the inside box wall in MM
internalboxLengthYMm = 90; // .1
// The internal height on the box top
internalBoxTopHeightZMm = 18; //25; // .1
// The internal height on the box bottom
internalboxBottomHeightZMm = 12; // .1
...
module BoxTop(isStdHinge) {
    difference() {
        union() {
            ...
            rotate([180,0,0]) translate([0,-boxLengthYMm,-openingTolerance]) 
                LatchMount(latchSupportTotalWidth, latchSupportWidth, latchScrewPositionPct, boxTopHeightZMm, latchSupportRadius, latchToloerance);
        ...
        rotate([180,0,0]) translate([0,-boxLengthYMm,-openingTolerance]) 
            ...
            translate([boxWidthXMm/2, boxLengthYMm-48, 28.1])
 
        linear_extrude(10) //boxWallWidthMm)
            square([120,88], true);
    }
 

    
    translate([boxWidthXMm/2, boxLengthYMm-48, 16]) //28.1])
        rotate([180,0,90])
        trap_base();
...
module BoxBottom(isStdHinge) {
    difference() {
        ...
        translate([-(boxWidthXMm/2), -(boxLengthYMm/2), -boxBottomHeightZMm*11]) 
            cube([boxWidthXMm*2,boxLengthYMm*2, boxBottomHeightZMm*10]);
        ...
        // TODO: Cut off anything in the inside of the case
        difference() {
            translate([0,0,-(boxBottomHeightZMm-boxWallWidthMm-.01)]) 
                cube([boxWidthXMm, boxLengthYMm, boxBottomHeightZMm-boxWallWidthMm-.02]);
            BoxShellBase(boxBottomHeightZMm, boxChamferRadiusMm);
        
        }
        
        // Cut out a hole for the base's content
        //translate([-(boxWidthXMm/2), -(boxLengthYMm/2), -boxBottomHeightZMm*11]) 
        //            //cube([boxWidthXMm*2,boxLengthYMm*2, boxBottomHeightZMm*20]);
        //translate([4, 2, -boxBottomHeightZMm]) 

                    translate([4, 4, -boxBottomHeightZMm])

            cube([120, 88, 10]);
                  //translate([boxWidthXMm/2, boxLengthYMm-48, 0])
 
        //linear_extrude(100) //boxWallWidthMm)
        //    square([122,92], true);
    ...
    translate([120/2+4, 88/2+4, -boxBottomHeightZMm])
        rotate([0,0,270])
