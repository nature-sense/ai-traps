...
"""
Convert a trained YOLOv8n ONNX model to RKNN format for Rockchip NPU (RK3566 / ROCK 3C).
...
Usage (local):
    python convert_rknn.py --input best.onnx --output yolov8n.rknn
...
Usage (Docker):
    docker run --rm -v $(pwd):/models rknn-converter-yolov8n \
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert YOLOv8n ONNX model to RKNN for Rockchip NPU"
    )
    parser.add_argument(
        "--input", "-i",
        type=str,
