...
int CameraHalImx415::alloc_dma_buf(uint32_t size) {
    const char* heap_dev = "/dev/dma_heap/system";
    int heap_fd = open(heap_dev, O_RDWR | O_CLOEXEC);
    if (heap_fd < 0) {
        std::cerr << "[CameraHalImx415] cannot open " << heap_dev << ": "
                  << std::strerror(errno) << "\n";
        return -1;
    }
    ...
    if (ioctl(heap_fd, DMA_HEAP_IOCTL_ALLOC, &data) < 0) {
        std::cerr << "[CameraHalImx415] DMA_HEAP_IOCTL_ALLOC failed: "
                  << std::strerror(errno) << "\n";
        close(heap_fd);
        return -1;
    }
...
void* CameraHalImx415::map_dma_buf(int fd, uint32_t size) {
    void* addr = mmap(nullptr, size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    if (addr == MAP_FAILED) {
        std::cerr << "[CameraHalImx415] mmap dma-buf failed: "
                  << std::strerror(errno) << "\n";
        return nullptr;
    }
    return addr;
...
void CameraHalImx415::unmap_dma_buf(void* addr, uint32_t size, int fd) {
...
bool CameraHalImx415::scale_nv12(const FrameBuffer& src, FrameBuffer& dst,
                                 int dst_w, int dst_h) {
    ...
    if (ret != IM_STATUS_SUCCESS) {
        std::cerr << "[CameraHalImx415] imresize failed: " << ret
                  << " (" << dst_w << "x" << dst_h << ")\n";
        return false;
    }
...
static XCamReturn aiq_err_cb(rk_aiq_err_msg_t* msg) {
    if (msg->err_code == XCAM_RETURN_BYPASS) {
        std::cerr << "[CameraHalImx415] AIQ fatal error, should quit\n";
    }
...
static XCamReturn aiq_sof_cb(rk_aiq_metas_t* meta) {
    static int cnt = 0;
    if (++cnt <= 2)
        std::cout << "[CameraHalImx415] AIQ SOF frame_id=" << meta->frame_id << "\n";
    return XCAM_RETURN_NO_ERROR;
}

bool CameraHalImx415::init_aiq(const std::string& iq_dir) {
    ...
    if (xret != XCAM_RETURN_NO_ERROR) {
        std::cerr << "[CameraHalImx415] enumStaticMetasByPhyId failed: " << xret << "\n";
        return false;
    }
    const char* sns_name = static_info.sensor_info.sensor_name;
    std::cout << "[CameraHalImx415] AIQ sensor: " << sns_name << "\n";

    rk_aiq_uapi2_sysctl_preInit_devBufCnt(sns_name, "rkraw_rx", 2);

    // Try scene preinit, but don't fail if it's not supported
    std::cerr << "[CameraHalImx415] Setting scene: normal" << std::endl;
    int scene_ret = rk_aiq_uapi2_sysctl_preInit_scene(sns_name, "normal", "");
    if (scene_ret < 0) {
        std::cerr << "[CameraHalImx415] rk_aiq_uapi2_sysctl_preInit_scene failed: "
                  << scene_ret << " (continuing anyway)" << std::endl;
    }

    std::cerr << "[CameraHalImx415] rk_aiq_uapi2_sysctl_init sns_name=" << sns_name << " iq_dir=" << iq_dir << std::endl;
    aiq_ctx_ = rk_aiq_uapi2_sysctl_init(sns_name, iq_dir.c_str(), aiq_err_cb, aiq_sof_cb);
    if (!aiq_ctx_) {
        std::cerr << "[CameraHalImx415] rk_aiq_uapi2_sysctl_init failed\n";
        return false;
    }

    xret = rk_aiq_uapi2_sysctl_prepare(aiq_ctx_, 0, 0, RK_AIQ_WORKING_MODE_NORMAL);
    if (xret != XCAM_RETURN_NO_ERROR) {
        std::cerr << "[CameraHalImx415] rk_aiq_uapi2_sysctl_prepare failed: " << xret << "\n";
        return false;
    }
    std::cout << "[CameraHalImx415] rk_aiq_uapi2_sysctl_prepare succeeded\n";

    xret = rk_aiq_uapi2_sysctl_start(aiq_ctx_);
    if (xret != XCAM_RETURN_NO_ERROR) {
        std::cerr << "[CameraHalImx415] rk_aiq_uapi2_sysctl_start failed: " << xret << "\n";
        return false;
    }

    std::cout << "[CameraHalImx415] RK_AIQ started\n";

    // Wait for ISP to converge before capturing
    std::this_thread::sleep_for(std::chrono::milliseconds(500));
    rk_aiq_uapi2_setAeLock(aiq_ctx_, true);
    std::cout << "[CameraHalImx415] AE locked\n";
    rk_aiq_uapi2_lockAWB(aiq_ctx_);
    std::cout << "[CameraHalImx415] AWB locked\n";
...
bool CameraHalImx415::init_v4l2() {
    const char* dev_name = "/dev/video0";

    v4l2_fd_ = open(dev_name, O_RDWR | O_NONBLOCK, 0);
    if (v4l2_fd_ < 0) {
        std::cerr << "[CameraHalImx415] cannot open " << dev_name << ": "
                  << std::strerror(errno) << "\n";
        return false;
    }
    std::cout << "[CameraHalImx415] opened " << dev_name << " (fd=" << v4l2_fd_ << ")\n";

    // ── Query capabilities ───────────────────────────────────────────────────
    struct v4l2_capability cap;
    std::memset(&cap, 0, sizeof(cap));
    if (ioctl(v4l2_fd_, VIDIOC_QUERYCAP, &cap) < 0) {
        std::cerr << "[CameraHalImx415] VIDIOC_QUERYCAP failed: " << std::strerror(errno) << "\n";
        close(v4l2_fd_);
        v4l2_fd_ = -1;
        return false;
    }
    std::cout << "[CameraHalImx415] driver=" << cap.driver
              << " card=" << cap.card
    ...
    if (ioctl(v4l2_fd_, VIDIOC_S_FMT, &fmt) < 0) {
        std::cerr << "[CameraHalImx415] VIDIOC_S_FMT failed: " << std::strerror(errno) << "\n";
        close(v4l2_fd_);
        v4l2_fd_ = -1;
        return false;
    }
    std::cout << "[CameraHalImx415] S_FMT: " << fmt.fmt.pix_mp.width << "x"
              << fmt.fmt.pix_mp.height << " stride="
              << fmt.fmt.pix_mp.plane_fmt[0].bytesperline << "\n";

    const __u32 frame_size = fmt.fmt.pix_mp.plane_fmt[0].sizeimage;

    // ── Use MMAP buffers (rkisp_v5 on this kernel doesn't support DMABUF for 1920x1080) ──
    std::cout << "[CameraHalImx415] using MMAP buffer mode\n";
    return init_v4l2_mmap(fmt, frame_size);
}

// ─── init_v4l2_mmap (fallback) ───────────────────────────────────────────────

bool CameraHalImx415::init_v4l2_mmap(const struct v4l2_format& fmt, __u32 frame_size) {
    (void)fmt;
    ...
    if (ioctl(v4l2_fd_, VIDIOC_REQBUFS, &req) < 0) {
        std::cerr << "[CameraHalImx415] VIDIOC_REQBUFS (MMAP) failed: "
                  << std::strerror(errno) << "\n";
        close(v4l2_fd_);
        v4l2_fd_ = -1;
        return false;
    }
    std::cout << "[CameraHalImx415] REQBUFS (MMAP): count=" << req.count << "\n";
    ...
    for (unsigned int i = 0; i < req.count; ++i) {
        ...
        if (ioctl(v4l2_fd_, VIDIOC_QUERYBUF, &buf) < 0) {
            std::cerr << "[CameraHalImx415] VIDIOC_QUERYBUF[" << i
        ...
        if (mapped == MAP_FAILED) {
            std::cerr << "[CameraHalImx415] mmap[" << i << "] failed: "
                      << std::strerror(errno) << "\n";
            close(v4l2_fd_);
            v4l2_fd_ = -1;
            return false;
    ...
    for (auto& slot : v4l2_buf_pool_) {
        ...
        if (ioctl(v4l2_fd_, VIDIOC_QBUF, &qbuf) < 0) {
            std::cerr << "[CameraHalImx415] VIDIOC_QBUF[" << slot.index
                      << "] failed: " << std::strerror(errno) << "\n";
            close(v4l2_fd_);
            v4l2_fd_ = -1;
            return false;
    ...
    if (ioctl(v4l2_fd_, VIDIOC_STREAMON, &buf_type) < 0) {
        std::cerr << "[CameraHalImx415] VIDIOC_STREAMON failed: "
                  << std::strerror(errno) << "\n";
        close(v4l2_fd_);
        v4l2_fd_ = -1;
        return false;
    ...
    use_dmabuf_ = false;
    std::cout << "[CameraHalImx415] V4L2 MMAP streaming started\n";
    return true;
}
...
bool CameraHalImx415::init_scaler_buffers() {
    ...
    std::cout << "[CameraHalImx415] scaler buffers allocated: "
...
bool CameraHalImx415::init(const PipelineConfig& cfg) {
    cfg_ = cfg;

    if (!init_aiq(cfg_.camera.iq_dir)) {
        std::cerr << "[CameraHalImx415] AIQ init failed\n";
        return false;
    }

    if (!init_v4l2()) {
        std::cerr << "[CameraHalImx415] V4L2 init failed\n";
        return false;
    }

    if (!init_scaler_buffers()) {
        std::cerr << "[CameraHalImx415] scaler buffer init failed\n";
        return false;
    }

    initialised_ = true;
    std::cout << "[CameraHalImx415] ready\n";
    return true;
}

// ─── acquire_frames ──────────────────────────────────────────────────────────

bool CameraHalImx415::acquire_frames(FrameBuffer& full, FrameBuffer& medium,
                                     FrameBuffer& lores) {
    acquire_frames_impl();
...
void CameraHalImx415::acquire_frames_impl() {
    // Reset frame buffers
    ...
    if (poll_ret <= 0) {
        if (poll_ret == 0) {
            std::cerr << "[CameraHalImx415] V4L2 poll timeout\n";
        } else {
            std::cerr << "[CameraHalImx415] V4L2 poll error: " << std::strerror(errno) << "\n";
        }
        return;
    }

    if (ioctl(v4l2_fd_, VIDIOC_DQBUF, &buf) < 0) {
        std::cerr << "[CameraHalImx415] VIDIOC_DQBUF failed: " << std::strerror(errno) << "\n";
        return;
    }
    ...
    if (it == v4l2_buf_pool_.end()) {
        std::cerr << "[CameraHalImx415] unknown buffer index " << buf.index << "\n";
...
void CameraHalImx415::release_frames() {
    release_frames_impl();
}

void CameraHalImx415::release_frames_impl() {
...
void CameraHalImx415::shutdown() {
    ...
    if (v4l2_fd_ >= 0) {
        ...
        close(v4l2_fd_);
        v4l2_fd_ = -1;
        std::cout << "[CameraHalImx415] V4L2 closed\n";
    }
    ...
    initialised_ = false;
    std::cout << "[CameraHalImx415] shutdown complete\n";
}
