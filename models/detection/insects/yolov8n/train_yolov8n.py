...
"""
Train a YOLOv8n insect detection model.

Uses yolov8n.pt from Ultralytics as the pretrained checkpoint and trains on
the Insect_Detect dataset (320×320, 1 class: "insect").

Usage:
    python train_yolov8n.py                          # default training
    python train_yolov8n.py --epochs 150 --batch 16  # custom params
    python train_yolov8n.py --device cpu             # CPU training
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Train YOLOv8n insect detection model"
    )
    parser.add_argument(
        "--data",
        type=str,
    ...
    parser.add_argument(
        "--name",
        type=str,
        default="yolov8n_insect",
        help="Run name (default: yolov8n_insect)",
    )
    parser.add_argument(
        "--resume",
        type=str,
    ...
    # ── Load model ───────────────────────────────────────────────────────────
    print(f"[INFO] Loading yolov8n.pt pretrained model")
