...
void CameraHalOv5647::unmap_dma_buf(void* addr, uint32_t size, int fd) {
...
static XCamReturn aiq_err_cb(rk_aiq_err_msg_t* msg) {
    if (msg->err_code == XCAM_RETURN_BYPASS) {
        std::cerr << "[CameraHalOv5647] AIQ fatal error, should quit\n";
    }
...
static XCamReturn aiq_sof_cb(rk_aiq_metas_t* meta) {
    static int cnt = 0;
    if (++cnt <= 2)
        std::cout << "[CameraHalOv5647] AIQ SOF frame_id=" << meta->frame_id << "\n";
    return XCAM_RETURN_NO_ERROR;
}

bool CameraHalOv5647::init_aiq(const std::string& iq_dir) {
    ...
    if (xret != XCAM_RETURN_NO_ERROR) {
        std::cerr << "[CameraHalOv5647] enumStaticMetasByPhyId failed: " << xret << "\n";
        return false;
    }
    const char* sns_name = static_info.sensor_info.sensor_name;
    std::cout << "[CameraHalOv5647] AIQ sensor: " << sns_name << "\n";
    ...
    if (!aiq_ctx_) {
        std::cerr << "[CameraHalOv5647] rk_aiq_uapi2_sysctl_init failed\n";
        return false;
    }

    xret = rk_aiq_uapi2_sysctl_prepare(aiq_ctx_, 0, 0, RK_AIQ_WORKING_MODE_NORMAL);
    if (xret != XCAM_RETURN_NO_ERROR) {
        std::cerr << "[CameraHalOv5647] rk_aiq_uapi2_sysctl_prepare failed: " << xret << "\n";
        return false;
    }
    std::cout << "[CameraHalOv5647] rk_aiq_uapi2_sysctl_prepare succeeded\n";

    xret = rk_aiq_uapi2_sysctl_start(aiq_ctx_);
    if (xret != XCAM_RETURN_NO_ERROR) {
        std::cerr << "[CameraHalOv5647] rk_aiq_uapi2_sysctl_start failed: " << xret << "\n";
        return false;
    }

    std::cout << "[CameraHalOv5647] RK_AIQ started\n";

    // Wait for ISP to converge before capturing
    std::this_thread::sleep_for(std::chrono::milliseconds(500));
    rk_aiq_uapi2_setAeLock(aiq_ctx_, true);
    std::cout << "[CameraHalOv5647] AE locked\n";
    rk_aiq_uapi2_lockAWB(aiq_ctx_);
    std::cout << "[CameraHalOv5647] AWB locked\n";
...
bool CameraHalOv5647::init_v4l2() {
    const char* dev_name = "/dev/video0";

    v4l2_fd_ = open(dev_name, O_RDWR | O_NONBLOCK, 0);
    if (v4l2_fd_ < 0) {
        std::cerr << "[CameraHalOv5647] cannot open " << dev_name << ": "
                  << std::strerror(errno) << "\n";
        return false;
    }
    std::cout << "[CameraHalOv5647] opened " << dev_name << " (fd=" << v4l2_fd_ << ")\n";

    // ── Query capabilities ───────────────────────────────────────────────────
    struct v4l2_capability cap;
    std::memset(&cap, 0, sizeof(cap));
    if (ioctl(v4l2_fd_, VIDIOC_QUERYCAP, &cap) < 0) {
        std::cerr << "[CameraHalOv5647] VIDIOC_QUERYCAP failed: " << std::strerror(errno) << "\n";
        close(v4l2_fd_);
        v4l2_fd_ = -1;
        return false;
    }
    std::cout << "[CameraHalOv5647] driver=" << cap.driver
              << " card=" << cap.card
    ...
    if (ioctl(v4l2_fd_, VIDIOC_S_FMT, &fmt) < 0) {
        std::cerr << "[CameraHalOv5647] VIDIOC_S_FMT failed: " << std::strerror(errno) << "\n";
        close(v4l2_fd_);
        v4l2_fd_ = -1;
        return false;
    }
    std::cout << "[CameraHalOv5647] S_FMT: " << fmt.fmt.pix_mp.width << "x"
...
bool CameraHalOv5647::init_v4l2_mmap(const struct v4l2_format& fmt, __u32 frame_size) {
    ...
    if (ioctl(v4l2_fd_, VIDIOC_REQBUFS, &req) < 0) {
        std::cerr << "[CameraHalOv5647] VIDIOC_REQBUFS (MMAP) failed: "
                  << std::strerror(errno) << "\n";
        close(v4l2_fd_);
        v4l2_fd_ = -1;
        return false;
    }
    std::cout << "[CameraHalOv5647] REQBUFS (MMAP): count=" << req.count << "\n";
    ...
    for (unsigned int i = 0; i < req.count; ++i) {
        ...
        if (ioctl(v4l2_fd_, VIDIOC_QUERYBUF, &buf) < 0) {
            std::cerr << "[CameraHalOv5647] VIDIOC_QUERYBUF[" << i
                      << "] failed: " << std::strerror(errno) << "\n";
            close(v4l2_fd_);
            v4l2_fd_ = -1;
            return false;
        ...
        if (mapped == MAP_FAILED) {
            std::cerr << "[CameraHalOv5647] mmap[" << i << "] failed: "
                      << std::strerror(errno) << "\n";
            close(v4l2_fd_);
            v4l2_fd_ = -1;
            return false;
    ...
    use_dmabuf_ = false;
    std::cout << "[CameraHalOv5647] V4L2 MMAP streaming started\n";
    return true;
}
...
bool CameraHalOv5647::init_scaler_buffers() {
    ...
    std::cout << "[CameraHalOv5647] scaler buffers allocated: "
...
bool CameraHalOv5647::init(const PipelineConfig& cfg) {
    cfg_ = cfg;

    if (!init_aiq(cfg_.camera.iq_dir)) {
        std::cerr << "[CameraHalOv5647] AIQ init failed\n";
        return false;
    }

    if (!init_v4l2()) {
        std::cerr << "[CameraHalOv5647] V4L2 init failed\n";
        return false;
    }

    if (!init_scaler_buffers()) {
        std::cerr << "[CameraHalOv5647] scaler buffer init failed\n";
        return false;
    }

    initialised_ = true;
    std::cout << "[CameraHalOv5647] ready\n";
    return true;
}

// ─── acquire_frames ──────────────────────────────────────────────────────────

bool CameraHalOv5647::acquire_frames(FrameBuffer& full, FrameBuffer& medium,
                                     FrameBuffer& lores) {
    acquire_frames_impl();
...
void CameraHalOv5647::acquire_frames_impl() {
    // Reset frame buffers
    ...
    if (poll_ret <= 0) {
        if (poll_ret == 0) {
            std::cerr << "[CameraHalOv5647] V4L2 poll timeout\n";
        } else {
            std::cerr << "[CameraHalOv5647] V4L2 poll error: " << std::strerror(errno) << "\n";
        }
        return;
    }

    if (ioctl(v4l2_fd_, VIDIOC_DQBUF, &buf) < 0) {
        std::cerr << "[CameraHalOv5647] VIDIOC_DQBUF failed: " << std::strerror(errno) << "\n";
        return;
    }
    ...
    if (it == v4l2_buf_pool_.end()) {
        std::cerr << "[CameraHalOv5647] unknown buffer index " << buf.index << "\n";
...
void CameraHalOv5647::release_frames() {
    release_frames_impl();
}

void CameraHalOv5647::release_frames_impl() {
...
void CameraHalOv5647::shutdown() {
    ...
    if (v4l2_fd_ >= 0) {
        ...
        close(v4l2_fd_);
        v4l2_fd_ = -1;
        std::cout << "[CameraHalOv5647] V4L2 closed\n";
    }
    ...
    initialised_ = false;
    std::cout << "[CameraHalOv5647] shutdown complete\n";
}
